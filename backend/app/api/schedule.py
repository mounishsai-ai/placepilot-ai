...
async def _run_negotiation_agent_bg(run_id: str, drive_id: str):
    """Runs in background with its OWN DB session (request session will be closed)."""
    async with async_session_factory() as db:
        try:
            await orchestrator.execute_run(db, run_id, drive_id)
        except Exception as e:
            logger.error(f"Negotiation run {run_id} failed for drive {drive_id}: {e}")
...
async def negotiate_schedule(
    ...
    """Start the negotiation agent: the TPO's scheduling agent proposes, the
    company's own agent reviews it against its panel's real signals (existing
    committed interviews, any stated availability) and either accepts or
    objects with a named panel and reason, and the two go back and forth
    until agreement or the round limit. Never writes a real slot itself --
    see negotiation_tools.py for why that's the whole isolation guarantee;
    POST .../negotiations/{run_id}/commit is the only thing that ever does,
    and only after a human reviews the outcome."""
    round_result = await db.execute(select(InterviewRound).where(InterviewRound.id == round_id))
    round_ = round_result.scalar_one_or_none()
    if not round_:
        raise HTTPException(status_code=404, detail="Round not found")
    ...
    if not matches_result.first():
        raise HTTPException(
            status_code=400,
            detail="No shortlisted students for this drive. Approve a shortlist first, then negotiate.",
        )

    run = await orchestrator.create_run(db, round_.drive_id, kind="negotiation", round_id=round_id)
    background_tasks.add_task(_run_negotiation_agent_bg, run.id, round_.drive_id)
    return {
...
@router.get("/rounds/{round_id}/negotiation")
async def get_round_negotiation(
    round_id: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_role(UserRole.TPO, UserRole.COMPANY)),
):
    """The latest negotiation run for this round, full trace included -- both
    the TPO and Company portals poll this by round_id. A company user only
    ever sees negotiations for their own company's drives; get_agent_run has
    no such scoping and is deliberately not reused here for that reason."""
    round_result = await db.execute(
        select(InterviewRound).options(selectinload(InterviewRound.drive)).where(InterviewRound.id == round_id)
    )
    round_ = round_result.scalar_one_or_none()
    if not round_:
        raise HTTPException(status_code=404, detail="Round not found")
    ...
    if current_user.role == UserRole.COMPANY:
        ...
        if not company or not round_.drive or round_.drive.company_id != company.id:
            raise HTTPException(status_code=403, detail="You can only view your own company's negotiations")

    runs_result = await db.execute(
        select(AgentRun)
        .where(AgentRun.drive_id == round_.drive_id)
        .order_by(AgentRun.created_at.desc())
    )
    run = next(
        (
            r for r in runs_result.scalars().all()
            if (r.state_json or {}).get("kind") == "negotiation" and (r.state_json or {}).get("round_id") == round_id
        ),
        None,
    )
    if not run:
    ...
    trace_result = await db.execute(
        select(AgentTrace).where(AgentTrace.run_id == run.id).order_by(AgentTrace.seq)
    )
    ...
    return {
        "run": {
            ...
            "final_proposal": (run.state_json or {}).get("final_proposal"),
...
@router.post("/negotiations/{run_id}/commit")
async def commit_negotiation(
    run_id: str,
    db: AsyncSession = Depends(get_db),
    _: object = Depends(require_role(UserRole.TPO)),
):
    """Writes the negotiated proposal as real interview slots. The negotiation
    loop itself never has a commit_schedule tool available to it -- this
    explicit, human-triggered endpoint is the only thing that ever does, and
    only once (an already-committed run can't be committed again)."""
    result = await db.execute(select(AgentRun).where(AgentRun.id == run_id))
    run = result.scalar_one_or_none()
    if not run:
        raise HTTPException(status_code=404, detail="Negotiation run not found")

    state = run.state_json or {}
    if state.get("kind") != "negotiation":
        raise HTTPException(status_code=400, detail="That run is not a negotiation")
    if run.status == AgentRunStatus.COMPLETED and state.get("committed"):
        raise HTTPException(status_code=400, detail="This negotiation was already committed")

    proposal = state.get("final_proposal")
    if not proposal:
        raise HTTPException(status_code=400, detail="No negotiated proposal to commit yet")
    ...
    for slot_data in proposal:
        db.add(InterviewSlot(
            round_id=round_id,
            student_id=slot_data["student_id"],
            panel_id=slot_data.get("panel_id"),
            room_id=slot_data.get("room_id"),
            slot_start=_as_naive(datetime.fromisoformat(slot_data["slot_start"].rstrip("Z"))),
            slot_end=_as_naive(datetime.fromisoformat(slot_data["slot_end"].rstrip("Z"))),
            status=SlotStatus.SCHEDULED,
        ))
    if round_.drive:
        round_.drive.status = DriveStatus.SCHEDULE_PENDING
    run.status = AgentRunStatus.COMPLETED
    run.state_json = {**state, "committed": True}
    await db.commit()

    logger.info(f"[{round_.drive_id}] committed {len(proposal)} negotiated slots for round {round_id}")
    return {"message": "Negotiated schedule committed", "committed_count": len(proposal), "round_id": round_id}


async def _run_onyx_agent_bg(run_id: str, drive_id: str):
    """Runs in background with its OWN DB session (request session will be closed)."""
    async with async_session_factory() as db:
        try:
            await orchestrator.execute_run(db, run_id, drive_id)
        except Exception as e:
            logger.error(f"Onyx run {run_id} failed for drive {drive_id}: {e}")


@router.post("/rounds/{round_id}/ask-onyx")
async def ask_onyx(
    round_id: str,
    background_tasks: BackgroundTasks,
    db: AsyncSession = Depends(get_db),
    ...
    """Start Onyx, the supervisor agent, for this round. Onyx doesn't propose or
    validate a schedule itself -- its tools dispatch the negotiation sub-agents
    (see onyx_tools.py / negotiation_tools.py) and read back what they actually
    did, then reports to the TPO. Runs in the background; the underlying
    negotiation's own trace is still reachable at GET .../negotiation exactly
    as before -- Onyx sits on top of it, it doesn't replace it."""
    round_result = await db.execute(select(InterviewRound).where(InterviewRound.id == round_id))
    round_ = round_result.scalar_one_or_none()
    if not round_:
        raise HTTPException(status_code=404, detail="Round not found")
    ...
    if not matches_result.first():
        raise HTTPException(
            status_code=400,
            detail="No shortlisted students for this drive. Approve a shortlist first, then ask Onyx.",
        )

    run = await orchestrator.create_run(db, round_.drive_id, kind="onyx", round_id=round_id)
    background_tasks.add_task(_run_onyx_agent_bg, run.id, round_.drive_id)
    return {
        "message": "Onyx dispatched",
        "drive_id": round_.drive_id,
        "round_id": round_id,
        "run_id": run.id,
    }
...
@router.get("/rounds/{round_id}/onyx")
async def get_round_onyx(
    round_id: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_role(UserRole.TPO, UserRole.COMPANY)),
):
    """The latest Onyx run for this round, full trace included -- same
    shape/scoping as GET .../negotiation, since the TPO schedule page polls
    both to render Onyx's dispatch line/report card around the negotiation
    arena."""
    round_result = await db.execute(
        select(InterviewRound).options(selectinload(InterviewRound.drive)).where(InterviewRound.id == round_id)
    )
    ...
    if current_user.role == UserRole.COMPANY:
        ...
        company = company_result.scalar_one_or_none()
        if not company or not round_.drive or round_.drive.company_id != company.id:
            raise HTTPException(status_code=403, detail="You can only view your own company's runs")
    ...
    run = next(
        (
            r for r in runs_result.scalars().all()
            if (r.state_json or {}).get("kind") == "onyx" and (r.state_json or {}).get("round_id") == round_id
        ),
        None,
    )
    if not run:
